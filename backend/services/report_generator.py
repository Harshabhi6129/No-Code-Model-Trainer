...
def _generate_detailed_insights(metrics: List[Dict], config: Dict) -> List[str]:
    """Generate detailed training insights using Gemini"""
    ...
    # Create a small sample of the metrics for the LLM
    n = len(metrics)
    indices = [0, n // 2, n - 1] if n > 2 else range(n)
    metrics_sample = [{k: v for k, v in metrics[i].items() if k in ['epoch', 'train_loss', 'val_loss', 'val_accuracy']} for i in indices]
    
    # Call Gemini
    report_data = generate_comprehensive_report(summary, config, metrics_sample)
    
    # Format the JSON output into a list of strings for the existing UI
    insights = []
    
    # Executive Summary
    insights.append(f"**Executive Summary**: {report_data.get('executive_summary', 'Analysis unavailable.')}")
    ...
    # Advice
    insights.append("**Tuning Advice**:")
    for advice in report_data.get('tuning_advice', []):
